...
public class LoginServiceImpl implements LoginService {
    ...
    @Override
    public Result<LoginResponse> activeLogin(String emailOrUid, String password) {
        ...
        loginResponse.setToken(JwtUtil.generateToken(userInfo));
    ...
    public Result<LoginResponse> autoLogin() {
        ...
        loginResponse.setToken(JwtUtil.generateToken(userInfo));
        if (loginResponse.getToken() == null) {
            Log.info(getClass(), "加密用户ID或token生成失败: {}", loginResponse.getToken());
            throw new OperationException(-1, "token生成失败");
        }
        return Result.success(loginResponse);
    }
